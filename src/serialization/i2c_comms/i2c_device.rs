...
pub trait I2CDeviceHandler<Device, T>: Send + Sync + UnwindSafe + RefUnwindSafe where Device: I2CDevice<T> {
    fn handle(&mut self, robot: &Robot, device: &mut Box<Device>, data: &T);
}
pub(crate) struct I2CDevicePair<Device, T> where Device: I2CDevice<T> {
    pub(crate) device: Box<Device>,
    pub(crate) handlers: Vec<Box<dyn I2CDeviceHandler<Device, T>>>
}
...
impl<Device, T> I2CConsumer for I2CDevicePair<Device, T> where Device: I2CDevice<T> {
    fn maybe_consume_packet(&mut self, robot: &Robot, packet: Packet) -> Option<Packet> {
