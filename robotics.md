...
FTC was designed to be an entry level robotics competition and its hardware reflects that. After some weirdness in the early days, its hardware was designed as follows: FIRST produced an "Expansion Hub," a dedicated device with 4 H-bridge motor drivers, quadrature motor encoders, i2c ports, servo ports, RS485 and a few others. This board (inside an enclosure, of course) would be supplemented with a team-procured Android phone. The phone would be connected via a physical USB serial connection to the Expansion Hub.
